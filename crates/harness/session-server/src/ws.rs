...
//! threaded into every JSON-RPC dispatch so a [`kangnam_harness_session_rpc::MessageGuard`]
...
async fn handle_socket(socket: WebSocket, ctx: Arc<ServerContext>, user: Option<UserContext>) {
    ...
    // Main loop: receive JSON-RPC → dispatch via session-rpc → reply.
    while let Some(Ok(msg)) = ws_receiver.next().await {
        match msg {
            Message::Text(text) => {
                let text_str: &str = &text;
                if let Ok(request) = serde_json::from_str::<JsonRpcRequest>(text_str) {
                    let make_sink = || -> Arc<dyn kangnam_harness_session_agent::AgentEventSink> {
                        Arc::new(BroadcastSink::new(
                            ctx.broadcast_tx.clone(),
                            Some(ctx.enhanced_broadcast_tx.clone()),
                        ))
                    };
                    let dispatch_ctx = kangnam_harness_session_rpc::DispatchContext {
                        cli_manager: &ctx.cli_manager,
                        db: &ctx.db,
                        pending_permissions: &ctx.pending_permissions,
                        // Optional suspend/resume maps supplied by the
                        // host. session-server only transports responses;
                        // tool execution lives outside this crate.
                    ...
                    let response =
                        kangnam_harness_session_rpc::dispatch(request, dispatch_ctx).await;
