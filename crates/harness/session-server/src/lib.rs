//! Axum WebSocket + MCP server for the session module.
//!
...
//!   via [`kangnam_harness_session_rpc::dispatch`] and forwards broadcast notifications
//!   to subscribers.
...
use kangnam_harness_session_agent::CliManager;
use kangnam_harness_session_rpc::{
...
pub struct ServerContext {
    ...
    /// [`kangnam_harness_session_rpc::MessageGuard`].
    pub message_guard: Option<Arc<dyn MessageGuard>>,
    /// Optional form-response map for host interaction bridges. `None`
    /// for hosts that do not expose interactive form tools.
    pub pending_question_forms: Option<Arc<AsyncMutex<PendingQuestionForms>>>,
    /// Optional preview-response map for host interaction bridges.
...
mod boundary_tests {
    #[test]
    fn server_does_not_depend_on_legacy_chat_crates() {
        let manifest = include_str!("../Cargo.toml");

        assert!(
            !manifest.contains("kangnam-chat"),
            "session-server should expose transport hooks without depending on legacy chat crates"
        );
    }
}
...
pub async fn start(
    ...
    eprintln!(
        "[session-server] Listening on ws://localhost:{}",
        config.port
    );
