 ...
 * tags inside chat-stream `text_delta` chunks.
 *
 * Ported verbatim from open-design `src/artifacts/parser.ts`
 * (Apache-2.0). Single artifact at a time, no nesting. The output
 * event shape is preserved so the consumer (Phase 5b-06) can
 * upgrade to the upstream parser later without rewriting.
 *
 * Usage:
 * ```ts
 * const parser = createArtifactParser()
 * for (const ev of parser.feed(chunk1)) handle(ev)
 * for (const ev of parser.feed(chunk2)) handle(ev)
 * for (const ev of parser.flush()) handle(ev) // turn end
 * ```
 *
 * The `feed()` generator holds back tail bytes that *might* be the
 * start of an open tag (e.g. `<art`) or the start of `</artifact>`,
 * so partial-chunk inputs never emit a malformed event.
 */
...
export function createArtifactParser(): ArtifactParser {
