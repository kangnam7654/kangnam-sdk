...
Rider-Waite-Smith tarot calculation engine — 78-card deck identity and spread
drawing. Pure Rust, no IO.

## Purpose

Library for drawing tarot spreads (one-card / three-card / Celtic cross) from the Rider-Waite-Smith deck and returning card identity, orientation, spread, and audit facts. User-facing interpretation text belongs in the consuming backend/service layer. Includes a Korean five-element (오행) mapping for apps that blend tarot with Korean fortune systems. Pure computation — no network, no database.
...
- `TarotCard`, `DrawnCard` — typed card identity and draw structs.
