...
//! [`crate::sink::AgentEventSink`] — session-rpc / session-server don't
...
//! [`kangnam_harness_session_core::Storage`] backend, calls the provider, streams the
...
//! pluggable [`kangnam_harness_session_core::Storage`] trait introduced in session-core
...
use kangnam_harness_session_core::{NewMessage, Storage};
use kangnam_router::{
    ChatMessage, LlmProviderDyn, LlmStreamEvent, ProviderConfig, Router,
...
/// session-rpc wraps both into the same `JsonRpcError::internal`.
...
impl ProviderFactory for StaticConfig {
    fn create(&self) -> Result<Box<dyn LlmProviderDyn>, String> {
        ProviderConfig::from(self)
            .create()
            .map_err(|e| e.to_string())
    }

    async fn context_window_budget(&self) -> Option<ContextWindowBudget> {
        provider_config_context_window_budget(&ProviderConfig::from(self)).await
    }
}

impl From<&StaticConfig> for ProviderConfig {
...
#[async_trait]
impl ProviderFactory for ProviderConfig {
    fn create(&self) -> Result<Box<dyn LlmProviderDyn>, String> {
        ProviderConfig::create(self).map_err(|e| e.to_string())
    }

    async fn context_window_budget(&self) -> Option<ContextWindowBudget> {
        provider_config_context_window_budget(self).await
    }
}

/// Adapter that lets [`HttpSessionManager`] use a multi-provider
/// [`kangnam_router::Router`] while still selecting one provider per session
/// turn.
#[derive(Clone)]
pub struct RouterProviderFactory {
    pub router: Router,
    pub provider_alias: String,
}

impl RouterProviderFactory {
    pub fn new(router: Router, provider_alias: impl Into<String>) -> Self {
        Self {
            router,
...
#[async_trait]
impl ProviderFactory for RouterProviderFactory {
    fn create(&self) -> Result<Box<dyn LlmProviderDyn>, String> {
        self.router
            .create_provider(&self.provider_alias)
            .map_err(|e| e.to_string())
    }

    async fn context_window_budget(&self) -> Option<ContextWindowBudget> {
        provider_config_context_window_budget(self.router.provider_config(&self.provider_alias)?)
            .await
    }
}

async fn provider_config_context_window_budget(
    config: &ProviderConfig,
) -> Option<ContextWindowBudget> {
...
mod tests {
    ...
    async fn fresh_storage() -> Arc<dyn Storage> {
        let s = kangnam_harness_session_core::sqlite_storage::SqliteStorage::in_memory().unwrap();
    ...
    #[test]
    fn provider_config_can_act_as_provider_factory() {
        let config = ProviderConfig::new("dummy", "", "", "");
        let provider = <ProviderConfig as ProviderFactory>::create(&config).unwrap();

        assert_eq!(provider.provider_key(), "dummy");
    }

    #[tokio::test]
    async fn router_provider_factory_selects_registered_alias() {
        let router = Router::new()
            .with_provider("primary", ProviderConfig::new("dummy", "", "", ""))
            .with_provider("secondary", ProviderConfig::new("dummy", "", "", ""));
        let factory = RouterProviderFactory::new(router, "secondary");
        let storage = fresh_storage().await;
        let mgr = HttpSessionManager::new(storage.clone(), Arc::new(factory));
        let sink = VecSink::new();

        mgr.start_session("conv-1", "secondary", sink.clone())
            .await
            .unwrap();
        sink.drain();
        mgr.send_message("conv-1", "hello through router", sink.clone())
            .await
            .unwrap();
        ...
        assert!(
            events
                .iter()
                .any(|event| matches!(event, UnifiedMessage::TextDelta { .. }))
        );
        assert!(matches!(
            events.last(),
            Some(UnifiedMessage::TurnEnd { .. })
