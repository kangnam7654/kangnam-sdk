...
//!
//! ## Why this lives in chat-rpc, not chat-server
//!
...
//! The companion `AuthHook` is in chat-server because it only fires
//! during HTTP/WS upgrade — irrelevant for in-process transports.

use async_trait::async_trait;

use kangnam_chat_core::json_rpc::JsonRpcError;
...
/// the WS-upgrade layer's job (`kangnam_chat_server::AuthHook`); by the time
