...
fn handle_tools_list(state: &McpState, id: Option<serde_json::Value>) -> Json<McpResponse> {
    ...
    // do not expose host preview interactions and should not surface
    // the tool.
    if state.ctx.pending_previews.is_some() {
        tools.push(json!({
            "name": "preview",
            "description": "Render a host artifact in the sandboxed preview surface and return screenshot + console output. Suspends the agent turn until the host posts cli.previewResult.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "path": {
...
async fn handle_preview_call(
    ...
    // The map is opt-in; hosts without preview interactions should
    // return a clear unavailable-tool error.
